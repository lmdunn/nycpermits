...
permits_bk1 = pd.read_csv('boro_data/permits_bk1.csv')
...
#max_delt_lat = 1/690
#max_delt_long = 1/400
#entered manually to make it faster


# BROOKLYN (BK) 1
t0 = time.time()
print('Processing Brooklyn 1')
permits_bk1.loc[0, 'complaints2014_15'] = 0 #CHANGE YEAR
for i in permits_bk1.index:
    lat = permits_bk1.loc[i, 'latitude']
    long = permits_bk1.loc[i, 'longitude']
    point1 = [lat, long]
    complaint_count = 0
    for n in nypd_bk.index:
        ...
        if (abs(lat - n_lat)<= 1/690) or (abs(lat - n_long)<= 1/400):
            ...
            if distance <= .1:
                complaint_count += 1
    permits_bk1.loc[i, 'complaints2014_15'] = complaint_count #CHANGE YEAR

permits_bk1.to_csv('permits_complaints2014_bk1.csv', index = False) #CHANGE YEAR

print('')
print("Time to run Brooklyn1:", time.time()-t0)
...
for i in permits_bk2.index: #CHANGE PERMITS
...
permits_bk2.to_csv('permits_complaints2014_bk2.csv', index = False) #CHANGE YEAR
...
permits_bk3.to_csv('permits_complaints2014_bk3.csv', index = False)
